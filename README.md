...
**Run this before checkout:** clone the public repo, scan your local checkout, then use the Green/Yellow/Red result to decide whether the `$7` pack is actually useful for this agent run. `FREE_SCAN_FIRST_COMMAND_2026_07_07_2230`
...
**First five minutes after download:** [`docs/first-five-minutes-after-download.md`](docs/first-five-minutes-after-download.md) previews the local verifier and first repo preflight commands a buyer runs after Payhip. `FIRST_FIVE_MINUTES_AFTER_DOWNLOAD_2026_07_07`
...
**Live public distribution proof:** [`docs/live-distribution-proof.md`](docs/live-distribution-proof.md) gives a 60-second verification path through the free source repo, the merged Build with Claude route, CLA-green marketplace routes, the sentinel-changed harness-engineering route rechecked clean after a transient unknown state, the jqueryscript route rechecked clean by the public GitHub API after the 10:51 sentinel unknown-mergeability alert, the strict plugin route re-read by the public GitHub API as mergeable at head `7a4239ff0e30` with prescreen/prescreen-grade/Greptile success after the current-main rebase, the parked non-fit route, and the Yellow/Red buy trigger before checkout. Fresh readback markers: `LIVE_DISTRIBUTION_PROOF_REFRESH_2026_07_07_SENTINEL_1759` `STRICT_PLUGIN_ROUTE_MERGEABLE_READBACK_2026_07_07_1937` `STRICT_PLUGIN_ROUTE_GREEN_RECHECK_2026_07_08_0142` `LIVE_DISTRIBUTION_TRUST_REFRESH_2026_07_08_0412` `HARNESS_ROUTE_CLEAN_RECHECK_2026_07_08_0735` `STRICT_PLUGIN_ROUTE_PUBLIC_API_GREEN_RECHECK_2026_07_08_0843` `JQUERYSCRIPT_ROUTE_CLEAN_RECHECK_2026_07_08_1053`
...
<!-- deadline-ragas-eval-dataset-workflows:end -->
...
<!-- deadline-camel-ai-multi-agent-proof:end -->
...
<!-- deadline-playwright-mcp-browser-agent-proof:start -->
- [Playwright MCP browser-agent preflight proof](docs/examples/preflight-before-playwright-mcp-browser-agents.md) — for Playwright MCP browser-agent teams using `microsoft/playwright-mcp` before real browser sessions, logged-in apps, localhost services, forms, admin surfaces, package scripts, or secret-adjacent config enter scope; `PLAYWRIGHT_MCP_BROWSER_AGENT_PROOF`; Yellow/Red receipts point to the $7 pack: https://payhip.com/b/1nmxV
<!-- deadline-playwright-mcp-browser-agent-proof:end -->
...
- [FastMCP tool-server preflight proof](docs/examples/preflight-before-fastmcp-tool-servers.md) — for teams using `PrefectHQ/fastmcp` or FastMCP-style Python MCP servers before exposing `.mcp.json`, package scripts, API tools, env values, or shell commands to Claude/Cursor/Codex agents; `FASTMCP_TOOL_SERVER_PROOF`; Yellow/Red receipts point to the $7 pack: https://payhip.com/b/1nmxV
...
## Graphiti agent-memory teams proof
