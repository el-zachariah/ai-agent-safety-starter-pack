...
class LiveDistributionProofTests(unittest.TestCase):
    def test_live_distribution_proof_is_linked_and_specific(self):
        ...
        self.assertIn("LIVE_DISTRIBUTION_PROOF_REFRESH_2026_07_07_SENTINEL_1035", index)
        self.assertIn("LIVE_DISTRIBUTION_PROOF_REFRESH_2026_07_07_SENTINEL_1035", proof)
        ...
        self.assertIn("approved multi-harness agent listing", proof)
        self.assertIn("One route is merged", proof)
        self.assertIn("sixteen tracked routes are open and mergeable", proof)
        self.assertIn("now reads `OPEN` + `MERGEABLE` at head `d77a5d6`", proof)
        self.assertIn("review-progress evidence rather than endorsement", proof)
